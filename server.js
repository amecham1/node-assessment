...
String.prototype.bigFirstLetter = function() {
...
app.get('/api/users', function(req, res, next) {
    ...
    if (req.query.language) {
        returnData = users.filter(function(user) {
      ...
      returnData = users.filter(function(user){
        return user.city === req.query.city.bigFirstLetter();
      });
      res.json(returnData);
    }else if(req.query.state){
      returnData = users.filter(function(user){
        return user.state === req.query.state.bigFirstLetter();
      });
      res.json(returnData);
    }else if(req.query.gender){
      returnData = users.filter(function(user){
        return user.gender === req.query.gender.bigFirstLetter();
      });
      res.json(returnData);
    } else {
        res.json(users);
